...
for epoch in range(epochs):
    ...
    for features, emotion_labels, focus_labels in train_dataloader:
        ...
        total_emotion_accuracy += (predicted_emotion == emotion_labels).sum().item()
        total_focus_accuracy += (predicted_focus == focus_labels).sum().item()
    ...
    print(f"Epoch [{epoch+1}/{epochs}] Test Results - Emotion Accuracy: {emotion_acc:.4f}, Emotion F1 Score: {emotion_f1:.4f},"
          f"Focus Accuracy: {focus_acc:.4f}, Focus F1 Score: {focus_f1:.4f},"
    ...
    if avg_accuracy > best_avg_accuracy:
        ...
        best_epoch = epoch + 1
        torch.save(model.state_dict(), 'pth_normalized/best_ple_uncertainty_model.pth')

# 打印最佳结果
print(f"Best Model at Epoch [{best_epoch}] - "
      f"Emotion Accuracy: {best_emotion_acc:.4f}, Emotion F1 Score: {best_focus_f1:.4f},"
      f"Focus Accuracy: {best_focus_acc:.4f}, Focus F1 Score: {best_focus_f1:.4f},"
