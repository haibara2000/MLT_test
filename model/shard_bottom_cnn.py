...
class SharedBottomCNNModel(nn.Module):
    def __init__(self, emotion_output_dim, focus_output_dim):
        ...
        self.shared_bottom = nn.Sequential(
            nn.Conv2d(1, 16, kernel_size=3, stride=1, padding=1),  # 输入 (1, 2, 30)，输出 (16, 2, 30)
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=(2, 2)),  # 输出 (16, 1, 15)
            nn.Conv2d(16, 32, kernel_size=3, stride=1, padding=1),  # 输出 (32, 1, 15)
            nn.ReLU(),
            nn.Flatten()  # 输出 (32 * 15 = 480)
        )
        shared_output_dim = 32 * 15  # Flatten 后的维度
